...
enum PetSpritePose: String, CaseIterable, Hashable, Sendable {
    ...
    /// Procedural motion for a character that ships ONE frame.
    ///
    /// The three bundled Jedi besides Miles Windu, every OpenPets still, and any
    /// PNG dropped on the pet map a single image onto all ten poses, so they sat
    /// completely motionless under an "Advanced" badge. Rather than author ten
    /// strips each, a still figure gets a weight-shift cadence: bob on the feet,
    /// lean out of phase with it, and a small step.
    ///
    /// Intensity ESCALATES with session load, which is what keeps the count
    /// legible from the figure itself now that there are no droids to count.
    /// Calm states stay calm and return nil rather than fidgeting.
    ///
    /// Authored strips never reach this. Shaking a real animation reads as a
    /// rendering fault, not as motion.
    var stillCadence: (bob: CGFloat, lean: Double, step: CGFloat, rate: Double)? {
        switch self {
        case .idle, .waiting, .stopped, .thinking, .reading, .writing: nil
        case .done: (bob: 0.030, lean: 1.6, step: 0.002, rate: 2.1)
        case .patrol: (bob: 0.026, lean: 2.2, step: 0.005, rate: 2.6)
        case .working, .searching, .grepping: (bob: 0.034, lean: 2.4, step: 0.003, rate: 3.4)
        case .error, .attention: (bob: 0.030, lean: 3.0, step: 0.002, rate: 5.2)
        // Two, three, then five droids used to carry the load reading. With one
        // still frame the cadence has to carry it instead, so each rung is
        // faster and wider than the one below it.
        case .duel: (bob: 0.038, lean: 3.6, step: 0.005, rate: 3.9)
        case .trio: (bob: 0.048, lean: 5.0, step: 0.007, rate: 4.8)
        case .swarm: (bob: 0.058, lean: 6.6, step: 0.009, rate: 5.9)
        }
    }

}

/// Weight-shift motion for a character that ships ONE frame per pose.
enum PetStillMotion {
    struct Offset: Equatable {
        ...
        var isStill: Bool { bob == 0 && lean == 0 && step == 0 }
    }

    /// Lean runs at the base rate and bob at twice it, because feet plant twice
    /// per sway; matching the two makes the figure rock rigidly instead of
    /// stepping. The quarter-turn offset on step keeps the slide from peaking
    /// together with the lean, which is what reads as a sticker being dragged.
    ///
    /// `frameCount > 1` returns zero: an authored strip is a real animation and
    /// shaking it reads as a rendering fault.
    static func offset(
        pose: PetSpritePose,
        frameCount: Int,
        reduceMotion: Bool,
        ...
        guard !reduceMotion, frameCount <= 1, let cadence = pose.stillCadence else { return Offset() }
        let t = time * cadence.rate
        let reach = max(1, size * characterScale)
        return Offset(
            bob: CGFloat(sin(t * 2)) * reach * cadence.bob,
            lean: sin(t) * cadence.lean,
            step: CGFloat(sin(t + .pi / 2)) * reach * cadence.step
        )
    }

    /// Widest displacement the cadence can reach, as a fraction of the rendered
    /// figure. The pet card reserves a fixed viewport, so motion has to stay
    /// inside the art's own margins rather than growing the frame.
    static func peakFraction(_ pose: PetSpritePose) -> (bob: CGFloat, step: CGFloat) {
        guard let cadence = pose.stillCadence else { return (0, 0) }
        return (cadence.bob, cadence.step)
    }
}

extension PetSpritePose {
    var cinematic: Bool {
        switch self {
